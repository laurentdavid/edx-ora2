...
OpenAssessment.StudioView = function(runtime, element, server) {
    ...
    this.rubricXmlBox = CodeMirror.fromTextArea(live_element.find(".openassessment-rubric-editor").first().get(0), {
        mode: "xml",
        lineNumbers: true,
        lineWrapping: true
    });
    ...
    this.aiTrainingExamplesCodeBox = CodeMirror.fromTextArea(live_element.find("#ai-training-examples").get(0), {
        mode: "xml",
        lineNumbers: true,
        lineWrapping: true
    });
    this.studentTrainingExamplesCodeBox = CodeMirror.fromTextArea(live_element.find("#student-training-examples").get(0), {
        mode: "xml",
        lineNumbers: true,
        lineWrapping: true
    });
    var view = this;
    live_element.find(".openassessment-save-button").click(function(eventData) {
        view.save();
    });
    live_element.find(".openassessment-cancel-button").click(function(eventData) {
        view.cancel();
    });
...
OpenAssessment.StudioView.prototype = {
    load: function() {
        var view = this;
        this.server.loadEditorContext().done(function(prompt, rubricXml, title, sub_start, sub_due, assessments) {
